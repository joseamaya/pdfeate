    ...
    <div className="flex justify-between items-center bg-card border border-border rounded-xl p-3 mb-2">
      ...
      <div className="flex items-center gap-3 flex-shrink-0">
        {result.status === "completed" && result.id && (
          <a className="bg-success text-white rounded-md text-sm font-medium cursor-pointer px-4 py-2 no-underline transition-colors hover:bg-green-700 inline-block" href={getDownloadUrl(result.id)} download>
            Descargar ZIP
          </a>
        )}
        {result.status === "error" && (
